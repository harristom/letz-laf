...
<article class="event-card__card--container">
    ...
    <section class="event-card__photo--modify">
        <time class="event-card__time" datetime="{{ \Carbon\Carbon::parse($event->date)->format('d/m') }}">
            <span class="event-card__day">{{ \Carbon\Carbon::parse($event->date)->format('d') }}</span> <span class="event-card__month">{{ \Carbon\Carbon::parse($event->date)->format('M') }}</span>
        </time>
    ...
    <aside class="event-card__distance-and-participants--details">
        <p class="event-card__distance"><i class="fa-solid fa-person-running"></i>{{ number_format($event->distance_metres / 1000, 1) }} km</p>
        <p class="event-card__participants"><i class="fa-solid fa-user"></i>{{ count($event->participants) }}</p>
    </aside>
</article>
